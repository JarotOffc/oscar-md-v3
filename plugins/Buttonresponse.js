    ...
    async all(m, chatUpdate) {
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                let [menu, ...args] = noPrefix.trim().split` `.filter(v => v)
                menu = (menu || '').toLowerCase()
                let isId = plugin.command instanceof RegExp ? // RegExp Mode?
                    plugin.menu.test(menu) :
                    Array.isArray(plugin.menu) ? // Array?
                        plugin.menu.some(cmd => cmd instanceof RegExp ? // RegExp in Array?
                            cmd.test(menu) :
                            cmd === menu
                        ) :
                        typeof plugin.menu === 'string' ? // String?
                            plugin.menu === menu :
                            false
                if (!isId) continue
                console.log({ name, menu: plugin.menu, text: id })
                isIdMessage = true
            }
